...
const withConfig = fancy.add('config', () => Config.load(rootAsFileURL))
...
describe('esm', () => {
  ...
  withConfig.stdout().it('runs esm command and prerun & postrun hooks', async (ctx) => {
  ...
  withConfig.stdout().it('runs esm command, postrun hook captures command result', async (ctx) => {

...
describe('Config', () => {
  ...
  const findCommandTestConfig = ({pjson, homedir = '/my/home', platform = 'darwin', env = {}}: Options = {}) => {
    ...
    test = test.add('config', async () => {
      ...
      config.setEdge(config as Config, pluginA, 'core')
      config.setEdge(pluginA, commandPluginA, 'core')
      config.setEdge(config as Config, pluginB, 'core')
      config.setEdge(pluginB, commandPluginB, 'core')
      return config
    })
    // eslint-disable-next-line @typescript-eslint/ban-ts-ignore
    // @ts-ignore

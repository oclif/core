...
describe('styled/table', () => {
  ...
  .end('export flags and display()', () => {
    expect(typeof (CliUx.cli.table.flags())).to.eq('object')
    expect(typeof (CliUx.cli.table)).to.eq('function')
  })

  fancy
  .end('has optional flags', _ => {
    const flags = CliUx.cli.table.flags()
 ...
 321 supertable-test-2${ws}\n`)
  ...
  describe('columns', () => {
    ...
    .end('shows extended columns/uses get() for value', output => {
      CliUx.cli.table(apps, columns, {extended: true})
 ...
 321 supertable-test-2 https://supertable-test-2.herokuapp.com/ heroku-16${ws}\n`)
  ...
  describe('options', () => {
    ...
    .end('shows title with divider', output => {
      CliUx.cli.table(apps, columns, {title: 'testing'})
...
| 321 supertable-test-2${ws}\n`)
    ...
    .end('skips header', output => {
      CliUx.cli.table(apps, columns, {'no-header': true})
 ...
 321 supertable-test-2${ws}\n`)
    ...
    .end('only displays given columns', output => {
      CliUx.cli.table(apps, columns, {columns: 'id'})
 ...
 321${ws}\n`)
    ...
    .end('outputs in csv', output => {
      CliUx.cli.table(apps, columns, {output: 'csv'})
...
123,supertable-test-4\n`)
    ...
    .end('outputs in csv without headers', output => {
      CliUx.cli.table(apps, columns, {output: 'csv', 'no-header': true})
...
321,supertable-test-2\n`)
    ...
    .end('outputs in csv with alias flag', output => {
      CliUx.cli.table(apps, columns, {csv: true})
      expect(output.stdout).to.equal(`ID,Name
123,supertable-test-1
321,supertable-test-2\n`)
    })
    ...
    .end('outputs in json', output => {
      CliUx.cli.table(apps, columns, {output: 'json'})
...
`)
    ...
    .end('outputs in yaml', output => {
      CliUx.cli.table(apps, columns, {output: 'yaml'})
...
`)
    ...
    .end('sorts by property', output => {
      CliUx.cli.table(apps, columns, {sort: '-name'})
      expect(output.stdout).to.equal(` ID  Name${ws.padEnd(14)}
 ─── ─────────────────${ws}
 321 supertable-test-2${ws}
 123 supertable-test-1${ws}\n`)
    ...
    .end('filters by property & value (partial string match)', output => {
      CliUx.cli.table(apps, columns, {filter: 'id=123'})
      expect(output.stdout).to.equal(` ID  Name${ws.padEnd(14)}
 ─── ─────────────────${ws}
 123 supertable-test-1${ws}\n`)
    })
    ...
    .end('does not truncate', output => {
      ...
      CliUx.cli.table([...apps, three], columns, {filter: 'id=0', 'no-truncate': true})
 ...
 ${three.id} supertable-test-3${ws}\n`)
  ...
  describe('#flags', () => {
    ...
    .end('includes only flags', _ => {
      const flags = CliUx.cli.table.flags({only: 'columns'})
    ...
    .end('excludes except flags', _ => {
      const flags = CliUx.cli.table.flags({except: 'columns'})
  ...
  describe('edge cases', () => {
    ...
    .end('ignores header case', output => {
      CliUx.cli.table(apps, columns, {columns: 'iD,Name', filter: 'nAMe=supertable-test', sort: '-ID'})
      expect(output.stdout).to.equal(` ID  Name${ws.padEnd(14)}
 ─── ─────────────────${ws}
 321 supertable-test-2${ws}
 123 supertable-test-1${ws}\n`)
    ...
    .end('displays multiline cell', output => {
      ...
      CliUx.cli.table([...apps, app3 as any], columns, {sort: '-ID'})
      expect(output.stdout).to.equal(` ID  Name${ws.padEnd(14)}
 ─── ─────────────────${ws}
